$(document).ready(function () {
    ...
    function handleActiveSession(response) {
        examStarted = true;
        $('.btn-start-exam').prop('disabled', true).css('opacity', '1').text('Exam in Progress');
        if (response.endTime) {
    ...
    function handleStartExam(response) {
        if (response.message === "Exam data updated successfully" && response.examSessionActive === "yes") {
            ...
            // Access the first item in the currentexam array directly
            const examDetails = response.userExamData.currentexam[0]; // Assuming there's at least one exam in the array
            ...
            if (examDetails && examDetails.endTime) {
                ...
                disableUIComponents(); // Disable UI components as the exam details are missing
            }
        } else {
            console.log("No active exam session or user hasn't started an exam yet.");
