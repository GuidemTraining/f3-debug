    ...
    function sendExamActionRequest(action) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (action === 'start' && response.examDuration) {
                    examStarted = true;
                    // Assuming the response includes the exam duration in seconds
                    examEndTime = Date.now() + response.examDuration * 1000; // Convert seconds to milliseconds
                    console.log("Exam End Time:", new Date(examEndTime)); // Log the exam end time for debugging
