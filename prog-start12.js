$(document).ready(function () {
    ...
    function manageStartButtonBasedOnSession(examSessionActive) {
        if (examSessionActive === "yes") {
            $('.btn-start-exam').prop('disabled', true).css('opacity', '0.5').text('Good Luck!'); // Disable if exam is active
        } else {
            $('.btn-start-exam').prop('disabled', false).css('opacity', '').text('Start Exam'); // Enable if no active exam
        }
    }
    ...
    function fetchExamSessionStatus() {
        ...
        $.ajax({
            url: '/checkexamuserprogress', // Update with your server endpoint
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify(requestData),
            success: function(response) {
