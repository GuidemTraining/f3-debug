$(document).ready(function() {
    ...
    function enableUIComponents() {
        ...
        // Do not enable the "Start Exam" button here as its state is managed separately based on the exam session status
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.examSessionActive === "yes") {
                    disableUIComponents(); // Disable UI if exam is active
                } else {
                    enableUIComponents(); // Enable UI if no active exam
                }
