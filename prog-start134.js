$(document).ready(function () {
    ...
    function sendlabActionRequest(action) {
        ...
        const url = action === 'checklabuserprogress' ? 'https://sb1.guidem.ph/checklabuserprogress' : `https://sb1.guidem.ph/${action}-lab`;
...
$.ajax({
	...
	success: function (response) {
		...
		} else if (action === 'checklabuserprogress' && response.labSessionActive === "yes") {
...
function handleStartlab(response) {
    ...
    function fetchlabSessionStatus() {
        sendlabActionRequest('checklabuserprogress');
    }
