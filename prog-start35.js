$(document).ready(function () {
    ...
    function sendExamActionRequest(action) {
        ...
        const endpoint = action === 'checkExamUserProgress' ? `https://sb1.guidem.ph/checkExamUserProgress` : `https://sb1.guidem.ph/${action}-exam`;
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (action === 'start' || action === 'checkExamUserProgress') {
                    ...
                    updateExamTimer(); // Initialize or update timer
                    if (action === 'start') {
    ...
    // Function to handle reverting the exam (existing logic)

    // Function to handle rebooting the exam (existing logic)
    ...
    // Update UI elements based on the current state (existing logic)

    // Event listener for the start exam button (existing logic)

    // Initially fetch exam user progress when the page loads
    function fetchExamUserProgress() {
        sendExamActionRequest('checkExamUserProgress');
    }

    fetchExamUserProgress(); // Call this function to check exam progress initially
});
