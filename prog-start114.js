	...
	if (response.message === "Exam data updated successfully" && response.examSessionActive === "yes") {
		...
		if (examDetails && examDetails.endTime) {
			...
			if (!isNaN(parsedEndTime.getTime()) && parsedEndTime > new Date()) {
				...
				enableUIComponents();
				showNotificationModal(`Get Ready ${data.user.firstName}`,'The environment is being prepared and will be ready within 5 minutes. Please ensure to read the instructions carefully');
			} else {
