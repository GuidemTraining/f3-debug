$(document).ready(function () {
    ...
    function fetchExamSessionStatus() {
        var requestData = {
            userId: userId,
            courseId: courseId,
            chapterId: lessonId, // Assuming chapterId and lessonId are used interchangeably
            lessonId: lessonId
        };
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.examSessionActive === "yes") {
                    examStarted = true; // Update exam status
                    examEndTime = new Date(response.examData.endTime); // Update exam end time
                    examStartTime = new Date(response.examData.startTime); // Update exam start time
                    updateExamTimer(); // Initialize timer immediately
                    examTimerInterval = setInterval(updateExamTimer, 1000); // Update timer every second
                    enableUIComponents(); // Enable UI elements
                } else {
                    disableUIComponents(); // Disable UI elements
                }
    ...
    if (typeof (CoursePlayerV2) !== 'undefined') {
        CoursePlayerV2.on('hooks:contentDidChange', function (data) {
            ...
            fetchExamSessionStatus(); // Fetch exam session status whenever content changes
        });
    }
    ...
    function disableUIComponents() {
        ...
        $('.btn-revert-exam').prop('disabled', true);
        $('.btn-reboot-exam').prop('disabled', true);
    }

    // Function to enable UI components
    function enableUIComponents() {
        $('.btn-start-exam').prop('disabled', false);
        $('.btn-revert-exam').prop('disabled', false);
        $('.btn-reboot-exam').prop('disabled', false);
    }
    ...
    // Fetch exam session status initially when the page loads
