    ...
    const addCopyButton = function() {
        $("pre").each(function() {
          if (!$(this).siblings('.kapow-copy-btn').length) {
            var buttonHtml = $("<button class='kapow-copy-btn btn btn-sm' style='position: absolute; top: 0.5rem; right: 0.5rem; z-index: 10000; background-color: skyblue; color: white; border: none; border-radius: 0.25rem; padding: 0.25rem 0.75rem; font-size: 0.875rem; cursor: pointer; opacity: 1;'><i class='fa fa-clipboard' aria-hidden='true'></i></button>");
            buttonHtml.hover(function() {
              ...
              buttonHtml.click(function() {
                var button = $(this);
                var text = button.parent().find("pre").text();
                navigator.clipboard.writeText(text).then(function() {
                  button.find('i').removeClass('fa-clipboard').addClass('fa-check');
                  setTimeout(function() {
                    button.find('i').removeClass('fa-check').addClass('fa-clipboard');
                  }, 2000);
                }).catch(function(err) {
              ...
              $(this).wrap("<div style='position: relative;' class='copy-btn-wrapper'></div>").after(buttonHtml);
            }
          });
      
          $("gcode").each(function() {
            if (!$(this).siblings('.kapow-copy-icon').length) {
              var iconHtml = $("<i class='fa fa-clipboard kapow-copy-icon' aria-hidden='true' style='margin-left: 5px; cursor: pointer; font-size: 0.75rem; opacity: 0.5;'></i>");
      
              iconHtml.hover(function() {
                $(this).css('opacity', '1');
              }, function() {
                $(this).css('opacity', '1');
              });
      
              iconHtml.click(function() {
                var icon = $(this);
                var text = icon.parent().text();
                navigator.clipboard.writeText(text).then(function() {
