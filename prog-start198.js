$(document).ready(function () {
    ...
    function handleActiveSession(response) {
        if (response.labSessionStatus === "ended") {
            ...
            labStarted = true;
            $('.btn-start-lab').prop('disabled', true).css({'opacity': '1', 'color': '#11111a', 'background': 'mediumaquamarine', 'border-radius': '8px' }).text('🔓 Unlocked');
...
function handleStartlab(response) {
    if (response.message === "Lab started successfully" && response.labSessionStatus === "yes") {
        labStarted = true;
        $('.btn-start-lab').prop('disabled', true).css({ 'opacity': '1', 'color': 'white' }).text('');
        $('.timer-container').removeClass('disabled');
        if (response.endtime) {
