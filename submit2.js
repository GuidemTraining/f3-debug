$(document).ready(function () {
    ...
    $(document).on('click', '.guidem-button', function () {
        ...
        .then(data => {
            ...
            if (data.isCorrect) {
                ...
                incorrectAttempts++;
                // Handle incorrect attempt logic here...
            }
        })
        .catch(error => {
            toastr.error(`Error: ${error.message}`);
            clickedButton.prop('disabled', false).css('background-color', ''); // Re-enable button on error
        });
    });
    ...
    $(document).on('click', '.guidem-hint-button', function () {
        ...
        const questionId = form.data('question-id');
        // Define hints for different question IDs
        let hint = "No hint available for this question.";
        // Example: if (questionId === '1') { hint = "This is your hint for question 1."; }
