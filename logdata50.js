$(document).ready(function() {
    ...
    function addClipboardFunctionality() {
        $("pre").each(function() {
            var $this = $(this);
            var buttonHtml = $('<button class="clipboard-button btn btn-secondary btn-sm" style="position: absolute; top: 0.5rem; right: 0.5rem; z-index: 10000;color:skyblue;opacity:0.7;background:none; display: none;"><i class="fa fa-clipboard" aria-hidden="true"></i></button>').click(function(event) {
                var text = $this.text();
                ...
                event.stopPropagation(); // Prevents event from reaching the pre element
            });
            $this.before(buttonHtml);
            $this.hover(function() {
                buttonHtml.show();
            }, function() {
                if (!buttonHtml.is(':animated')) { // Check if button is not being animated
                    buttonHtml.fadeOut(1000); // Hides the button after 1 second when mouse moves away
                }
            });
