$(document).ready(function () {
    ...
    var lastActionTime = sessionStorage.getItem('lastActionTime');
    ...
    $(document).on('click', '.btn-revert-exam', function () {
        ...
        if (revertCount > 0 && examStarted && (currentTime - lastActionTime > 60000)) {
            $('#revertModal').modal('show');
        } else {
            alert("Revert count exceeded, exam not started, or action frequency limit reached.");
        }
    });
    ...
    $('#confirmRevert').on('click', function () {
        ...
        sessionStorage.setItem('lastActionTime', lastActionTime);
        updateButtonStates();
    ...
    $(document).on('click', '.btn-reboot-exam', function () {
        var currentTime = new Date().getTime();
        if (rebootCount > 0 && examStarted && (currentTime - lastActionTime > 60000)) {
            $('#rebootModal').modal('show');
        } else {
            alert("Reboot count exceeded, exam not started, or action frequency limit reached.");
        }
    });
    ...
    function updateButtonStates() {
        ...
        if (examStarted) {
            $('.btn-start-exam').removeClass('btn-primary').addClass('btn-success').prop('disabled', true).css('color', 'white').css('font-weight', 'bold').text('Good Luck!');
        } else {
            $('.btn-start-exam').removeClass('btn-success').addClass('btn-primary').prop('disabled', false).css('color', '').css('font-weight', '').text('Start Exam');
        }
    }
