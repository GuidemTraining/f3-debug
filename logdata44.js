$(document).ready(function() {
    ...
    function addClipboardButton() {
        $('pre').each(function() {
            var $pre = $(this);
            if (!$pre.next().hasClass('clipboard-button')) {
                var $clipboardButton = $('<button class="clipboard-button btn btn-secondary btn-sm" style="margin-top: 5px;"><i class="fa fa-clipboard"></i></button>');
                $clipboardButton.click(function() {
                    var text = $pre.text();
                    navigator.clipboard.writeText(text).then(() => {
                        $clipboardButton.html('<i class="fa fa-check"></i>');
                        setTimeout(() => {
                            $clipboardButton.html('<i class="fa fa-clipboard"></i>'); 
                        }, 2000);
                    }).catch(err => {
                        console.error('Failed to copy:', err);
                    });
                });
                $pre.after($clipboardButton);
            }
        });
    }
