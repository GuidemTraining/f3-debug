$(document).ready(function () {
    ...
    function handleStartExam(response) {
        // Update UI to reflect the exam has started
        examStarted = true;
        $('.btn-start-exam').prop('disabled', true).css({'opacity': '1', 'color': 'white'}).text('Exam in Progress');
        enableUIComponents(); // Ensure UI is enabled correctly at the start
    
        // Extract the endTime directly for "start-exam" action or find the matching exam for "checkexamuserprogress"
        let endTime;
        if (response.message === "Exam started successfully" && response.examData) {
            endTime = response.examData.endTime; // Directly from start exam response
        } else if (response.examSessionActive === "yes" && response.userExamData) {
            // Assuming userExamData contains the array currentexam with the structure as mentioned
            const matchedExam = response.userExamData.currentexam.find(exam => 
                exam.courseId === courseId && 
                exam.lessonId === lessonId);
            if (matchedExam) {
                endTime = matchedExam.endTime;
            }
        }
    
        if (endTime) {
            examEndTime = new Date(endTime);
            updateExamTimer(); // Start or update the exam timer
            if (!examTimerInterval) {
                examTimerInterval = setInterval(updateExamTimer, 1000);
            }
        } else {
            console.error('Failed to retrieve exam end time.');
            alert('There was an error with the exam setup. Please contact support.');
            disableUIComponents(); // Disable UI if the exam time couldn't be determined
        }
    }
