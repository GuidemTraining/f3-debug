$(document).ready(function () {
    ...
    $(document).on('click', '.btn-revert-exam', function () {
        if (revertCount > 0 && examStarted && !recentlyReverted()) {
            $('#revertModal').modal('show');
        } else {
            alert("Revert count exceeded, exam not started, or recently reverted.");
        }
    });
    ...
    function updateButtonStates() {
        $('.btn-revert-exam').text("Revert (" + revertCount + " left)").prop('disabled', !examStarted || revertCount <= 0 || recentlyReverted());
