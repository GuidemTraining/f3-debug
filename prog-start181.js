$(document).ready(function () {
    ...
    function enableUIComponents(response) {
        // Extract completed items from the response
        const completedItems = response.completedItems || [];
        
        // Iterate through each form to check its completion status
        $('.guidem-form').each(function() {
            var $form = $(this);
            var formQuestionId = $form.data('question-id').toString();
            
            // Check if the question has been completed
            if (!completedItems.includes(formQuestionId)) {
                // Enable UI components for not completed questions
                $form.find('.guidem-button, .guidem-hint-button').prop('disabled', false).css('opacity', '');
                $form.find('.guidem-master-container input[type="text"]')
                    .attr('placeholder', `${userFirstName} please submit your answer here ****`)
                    .prop('disabled', false); // Ensure input is enabled
                $form.find('.overlay').remove();
            } else {
                // For completed questions, update the placeholder and disable input
                $form.find('input[type="text"]').prop('disabled', true)
                    .attr('placeholder', `${userFirstName}, you already completed this. Grit & Grind!`)
                    .css({'color': 'skyblue', 'border': '1px solid skyblue'});
                    
                // Disable UI components for completed questions
                $form.find('.guidem-button, .guidem-hint-button').prop('disabled', true).css('opacity', '0.5');
            }        
        });
        
        // Additional UI adjustments as needed
        $('#accordionguidem .accordion-button').css({
            'pointer-events': 'auto',
            'opacity': '1',
            'user-select': 'auto'
        });
        // This line might be redundant and can cause confusion as it resets the placeholder
        // for all text inputs regardless of their completion status. Consider removing or adjusting it.
    ...
    function enableUIComponents(response) {
        // Ensure response is defined and contains completedItems
        if (response && response.completedItems) {
            // Extract completed items from the response
            const completedItems = response.completedItems.map(String);
            ...
            $('.guidem-form').each(function() {
                ...
                if (!completedItems.includes(formQuestionId)) {
                    // Enable UI components for not completed questions
                    $form.find('.guidem-button, .guidem-hint-button').prop('disabled', false).css('opacity', '');
                    $form.find('input[type="text"]').prop('disabled', false).attr('placeholder', `${userFirstName}, please submit your Answer Here ****`);
                    $form.find('.overlay').remove();
                } else {
                    // Set placeholder text and style for completed questions
                    $form.find('input[type="text"]').prop('disabled', true)
                        .attr('placeholder', `${userFirstName}, You already completed this. Grit & Grind!`)
                        .css({'color': 'skyblue', 'border': '1px solid skyblue'});
                }
            ...
            // Handle case where response is undefined or does not contain completedItems
            console.error("Response or completedItems is undefined.");
        }
    }
