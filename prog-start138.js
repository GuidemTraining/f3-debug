$(document).ready(function () {
    ...
    function sendlabActionRequest(action) {
        ...
        const url = `https://sb1.guidem.ph/${action}-lab`;
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.labSessionStatus === "yes") {
                    labStarted = true;
                    labEnded = false;
                    labEndTime = new Date(response.labData.endTime);
                    updatelabTimer();
                    if (!labTimerInterval) {
                    ...
                    disableUIComponents();
                    // Further actions for "ended" state
                }
            },
...
function handleStartlab(response) {
	if (response.message === "Lab started successfully" && response.LabSessionActive === "yes") {
		labStarted = true;
		$('.btn-start-lab').prop('disabled', true).css({'opacity': '1', 'color': 'white'}).text('lab in Progress');
		$('.timer-container').removeClass('disabled');
