$(document).ready(function () {
    ...
    .prop('disabled', true).css('opacity', '0.5');
        $('.guidem-master-container  .guidem-exam input[type="text"]').attr('placeholder', `${userFirstName} your exam session is not valid or has ended`);
        $('.guidem-master-container .accordion-collapse').removeClass('show');
        $('.guidem-master-container .accordion-button').addClass('collapsed').attr('aria-expanded', "false");
    ...
    if (examEnded) {
        ...
        showNotificationModal('Exam Ended', `GG! ${userFirstName} The exam duration has concluded. Please submit your report within 24 hours.`);
        $('#examTimer').text(`GG ${userFirstName}`);
        return; 
    }
    if (!examStarted) {
...
$.ajax({
	...
	success: function (response) {
		...
		if (action === 'start' && response.examSessionActive === "yes") {
			...
			showNotificationModal('Exam Ended', `Hi ${userFirstName} your exam has ended. Please submit your report.`);
      $('#examTimer').text(`GG ${userFirstName}`);
		} else {
...
function handleStartExam(response) {
	if (response.message === "Exam data updated successfully" && response.examSessionActive === "yes") {
		...
		$('.btn-start-exam').prop('disabled', true).css({'opacity': '1', 'color': 'white'}).text('Exam in Progress');
		$('.timer-container').removeClass('disabled');
		const examDetails = response.userExamData.currentexam.find(exam => exam.courseId === currentCourseId);
