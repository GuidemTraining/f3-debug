    ...
    function enableUIComponents(response) {
        ...
        $('.guidem-form').each(function() {
            ...
            if (!completedItems.includes(formQuestionId)) {
                ...
                $form.find('.guidem-master-container input[type="text"]').attr('placeholder', `${userFirstName} please submit your Answer Here ****`);
                ...
                // Find the nearest completed question form and disable its UI components
                var $nearestCompletedQuestionForm = $form.closest('.guidem-form[data-completed="true"]');
                $nearestCompletedQuestionForm.find('input[type="text"]').prop('disabled', true)
                    .attr('placeholder', `${userFirstName}, You already completed this. Grit & Grind!`);
                $nearestCompletedQuestionForm.find('.guidem-button, .guidem-hint-button').prop('disabled', true).css('opacity', '0.5');
            }
        });
