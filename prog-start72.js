$(document).ready(function () {
    ...
    function sendExamActionRequest(action) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (action === 'start' || (action === 'checkexamuserprogress' && response.examSessionActive === "yes")) {
                    examStarted = true;
                    // Convert the end time string to a Date object
                    examEndTime = new Date(response.examData ? response.examData.endTime : response.examEndTime);
                    updateExamTimer();
    ...
    function updateExamTimer() {
        ...
        if (timeLeft <= 0) {
            ...
            $('#examTimer').text(`${hours < 10 ? '0' + hours : hours}h ${minutes < 10 ? '0' + minutes : minutes}m ${seconds < 10 ? '0' + seconds : seconds}s`);
        }
    }
    ...
    // Initial fetch of exam session status
