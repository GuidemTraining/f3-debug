    ...
    function sendExamActionRequest(action) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (action === 'start') {
                    examStarted = true;
                    // Parse the endTime received from the server
                    ...
                    if (!isNaN(examEndTime.getTime())) {
                        ...
                        examEndTime.setTime(examEndTime.getTime() + response.examData.examDuration);
                        ...
                        console.error('Invalid end time received:', response.examData.endTime);
                        alert('Failed to start the exam due to an invalid end time. Please try again.');
                    }
                }
