$(document).ready(function() {
    ...
    function fetchAndDisplayScoreboard() {
        ...
        $.ajax({
            ...
            success: function(response) {
                // The response is expected to be an array, so we check the first element
    ...
    function updateChart(participants) {
        ...
        // Optionally add some padding to the max value
        const yAxisMax = maxScore + (maxScore * 0.1); // Adds 10% padding
    
        const datasets = participants.map((participant, index) => ({
            label: participant.username,
            data: [participant.score], // Replace with an array of scores if available
            fill: false,
        ...
        myChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: ['00:00', '00:10', '00:20', '00:30', '00:40'], // Replace with actual labels
                datasets: datasets
            },
            options: {
                responsive: true,
                scales: {
                    y: {
                        beginAtZero: true,
                        suggestedMax: yAxisMax // Use the computed max value
                    }
                },
                plugins: {
                    legend: {
